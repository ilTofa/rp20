...
- (void)trackGoal:(NSInteger)goal withRevenue:(double)revenue completionBlock:(void(^)(NSError* error))block {
  ...
  // Set goal id
  [params setValue:[NSString stringWithFormat:@"%ld", (long)goal] forKey:GOAL_ID];
...
- (void)cacheEvent:(NSMutableDictionary*)params completionBlock:(void(^)(NSError* error))block {
  ...
  dispatch_async(self.queue, ^ {
    ...
    [params setValue:[NSString stringWithFormat:@"%ld", (long)[dateComponents hour]] forKey:HOURS];
    [params setValue:[NSString stringWithFormat:@"%ld", (long)[dateComponents minute]] forKey:MINUTES];
    [params setValue:[NSString stringWithFormat:@"%ld", (long)[dateComponents second]] forKey:SECONDS];
    ...
    [params setValue:[NSString stringWithFormat:@"%ld", (long)self.totalNumberOfVisits] forKey:TOTAL_NUMNER_OF_VISIT];
...
- (void)emptyQueueWithCompletionBlock:(void (^)(NSError* error))block {
  ...
  dispatch_async(self.queue, ^ {
    ...
    if (events != nil && error == nil) {
      DLOG(@"Number of cached events to delete: %ld", (unsigned long)[events count]);
...
#else
      [self addVisitorScopeCustomVariableWithName:@"Platform" value:(__bridge NSString *)(SCDynamicStoreCopyComputerName(NULL, NULL)) withError:NULL];
      [self addVisitorScopeCustomVariableWithName:@"OS version" value:[[NSProcessInfo processInfo] operatingSystemVersionString] withError:NULL];
#endif
...
- (BOOL)addCustomVariableWithName:(NSString*)name value:(NSString*)value
  ...
  [customVariables addObject:[NSString stringWithFormat:@"\"%ld\":[\"%@\",\"%@\"]", (unsigned long)[customVariables count] + 1, name, value]];
