...
{	
	...
	switch (inPropertyID) {
		case kAudioFileStreamProperty_ReadyToProducePackets :
		{
			...
			//DLog(@"mFormatID: %c", asbd.mFormatID);
			...
			if (!err) {
				...
				for (int x = 0; x < formatListSize; x += sizeof(AudioFormatListItem)){
					...
					DLog(@"rate: %lf  Format:%lu%lu%lu%lu  FramesPerPacket:%lu  bytesPerFrame:%lu ChannelsperFrame:%lu\r\n",
		...
		if (length > 0)
		{
			...
			if (myData->metaDataInterval == 0)
			{
				...
				if (statusCode == 200)		// "OK" (this is true even for ICY)
				{
					...
					NSString *icyCheck = [[[NSString alloc] initWithBytes:bytes length:10 encoding:NSISOLatin1StringEncoding] autorelease];
			...
			if (myData.foundIcyStart && !myData.foundIcyEnd)
			{
					...
					while (YES)
					{
						...
						{		
							// get the full string
							NSString *fullString = [[[NSString alloc] initWithBytes:bytes length:streamStart encoding:NSISOLatin1StringEncoding] autorelease];
							
							// get the substring for this line
							NSString *line = [fullString substringWithRange:NSMakeRange(lineStart, (streamStart-lineStart))];
							DLog(@"Header Line: %@. Length: %d", line, [line length]);
							...
							if ([lineItems count] > 1)
							{
                                ...
                                {
                                    myData.streamRadioName = [[line substringFromIndex:10] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                                ...
                                {
                                    myData.streamGenre = [[line substringFromIndex:11] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
			...
			if (myData.parsedHeaders)
			{
				...
				for (int i=streamStart; i < length; i++)
				{
					...
					if (myData->metaDataInterval > 0 && myData->dataBytesRead == myData->metaDataInterval)
					{
						...
						//DLog(@"Found interval. Interval: %d, Meta Length: %d", myData->metaDataInterval, myData->metaDataBytesRemaining);
			...
			if (myData->discontinuous)
			{
				...
				{
					//DLog(@"Parsing no meta bytes (Discontinuous).");
				...
				{
					//DLog(@"Parsing normal bytes (Discontinuous).");
